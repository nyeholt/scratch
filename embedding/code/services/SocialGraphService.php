...
//			$response = $svc->request('', 'GET');
	 ...
	 * @param string $url 
	 */
	public function convertUrl($remoteUrl) {
		$oembed = Oembed::get_oembed_from_url($remoteUrl, false, $this->oembedOptions);
		if ($oembed) {
			return array('Title' => '', 'Content' => $oembed->forTemplate());
		} 

		$graph = OpenGraph::fetch($remoteUrl);
